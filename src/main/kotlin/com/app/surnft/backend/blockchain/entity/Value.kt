...
open class Value(val raw: String?) {
  ...
  fun fullFormat(none: String? = "", showSignMode: Int = com.app.surnft.backend.blockchain.entity.Value.Companion.MINUS_ONLY): String {
    return format(com.app.surnft.backend.blockchain.entity.Value.Companion.NONE_ROUND, none, showSignMode)
  }
  ...
  open fun format(decimalPlace: Int, empty: String?, showSignMode: Int): String {
    ...
    if (decimalPlace == com.app.surnft.backend.blockchain.entity.Value.Companion.NONE_ROUND) {
    ...
    if (result != null && result != empty) {
      if (showSignMode == com.app.surnft.backend.blockchain.entity.Value.Companion.MINUS_ONLY) {
        result = (if (value < BigDecimal.ZERO) "-" else "") + result
      } else if (showSignMode == com.app.surnft.backend.blockchain.entity.Value.Companion.ALL_SIGN) {
        result = (if (value >= BigDecimal.ZERO) "+" else "-") + result
      }
    }
    return result
